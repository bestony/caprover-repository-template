...
function configuredSiteUrl() {
    const value = normalizeBaseUrl(storeConfig.url);
    if (!value) {
        logger.error("apps", "config.js is missing url");
        throw new Error("config.js must export a non-empty url");
    }
    if (!/^https?:\/\//i.test(value)) {
        logger.error("apps", "config.js url is not an absolute http(s) URL", { url: value });
        throw new Error("config.js url must be an absolute http(s) URL");
    }
    return value;
}

function pathPrefixFromBaseUrl(baseUrl) {
    ...
    try {
        ...
        logger.debug("apps", "site url is not absolute, treating as path prefix", {
            baseUrl: trimmed,
            error: error.message,
        });
...
function storeBaseUrl(explicitBaseUrl) {
    if (explicitBaseUrl !== undefined) {
        const value = normalizeBaseUrl(explicitBaseUrl);
        logger.debug("apps", "resolved site base url", { source: "option", value });
        return value;
    }
    if (hasNonEmptyEnv("SITE_URL")) {
        const value = normalizeBaseUrl(process.env.SITE_URL);
        logger.debug("apps", "resolved site base url", { source: "env.SITE_URL", value });
        return value;
    }
    const value = configuredSiteUrl();
    logger.debug("apps", "resolved site base url", { source: "config.url", value });
    return value;
}

function storePathPrefix(explicitPathPrefix) {
    if (explicitPathPrefix !== undefined) {
        const value = normalizePathPrefix(explicitPathPrefix);
        logger.debug("apps", "resolved site path prefix", { source: "option", value });
        return value;
    }
    if (hasNonEmptyEnv("PATH_PREFIX")) {
        const value = normalizePathPrefix(process.env.PATH_PREFIX);
        logger.debug("apps", "resolved site path prefix", { source: "env.PATH_PREFIX", value });
        return value;
    }
    const value = pathPrefixFromBaseUrl(storeBaseUrl());
    logger.debug("apps", "resolved site path prefix", { source: "derived", value });
    return value;
}
...
function loadApps(options = {}) {
    const templatesDir = options.templatesDir || TEMPLATES_DIR;
    const publishedUrl = publishedBaseUrl(options);
    logger.info("apps", "using published site url", {
        baseUrl: publishedUrl,
        pathPrefix:
            options.pathPrefix !== undefined
                ? storePathPrefix(options.pathPrefix)
                : pathPrefixFromBaseUrl(publishedUrl),
        templatesDir,
    });
